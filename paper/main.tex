...
\begin{enumerate}
  \item Begin with information gathering to establish a shared foundation for the consultation.
...
The prototype is a web application deployed alongside the existing UpGrade demo app~\cite{upgrade-demo}. The prototype source is available in a public project repository.\footnote{Project repository: \url{https://github.com/CarnegieLearningWeb/ai-experiment-consultant}} Its footprint is minimal: a login page as a soft access guard, and a chat-style main page that supports file uploads (e.g., screenshots or documents) and opens the final report in a right-side panel from which it can be copied or downloaded. The backend is built around a single primary AI request endpoint; the initial prototype does not rely on a persistent database.
