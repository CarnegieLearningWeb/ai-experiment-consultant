...
\begin{enumerate}
  ...
  \item Keep humans in control through explicit approval at each major step.
  \item Produce a structured report useful to researchers, developers, product managers, and other stakeholders.
\end{enumerate}

The initial prototype focuses on the planning layer: it does not modify client code, create PRs, deploy changes, run real experiments, or make causal claims from synthetic data. It supports simple randomized designs with individual-level assignment, one decision point, and basic metrics. Future iterations of this tool will recommend more complex designs explore end-to-end automation. 

\section{Prototype Overview}

The prototype is a web application deployed alongside the existing UpGrade demo app~\cite{upgrade-demo}. Its footprint is minimal: a login page as a soft access guard, and a chat-style main page that supports file uploads (e.g., screenshots or documents) and displays the final report inline as a copyable markdown block. The backend is a single AI request endpoint; the initial prototype does not rely on a persistent database.
...
\node[phase, right=8mm of app]    (target) {Ideation\\Coaching};
...
\Description{Horizontal pipeline of six phases connected by arrows: Learning App, Page or Problem (both grouped under a dashed Context-collection label), Hypothesis Refinement, Experiment Design, Synthetic Preflight (dashed border, indicating optional), and Report Generation (drawn with a heavier border as the central artifact). Small boxed checkmarks sit on the arrows between Hypothesis Refinement and Experiment Design, and between Experiment Design and Synthetic Preflight, indicating user-approval gates. Below the Report Generation box, four arrows fan out via a horizontal bus to four italicized labels: Researchers, Developers, Stakeholders, and AI coding tools.}
\label{fig:workflow}
\end{figure}

\section{Consulting Workflow}

The flow has six phases, presented above in five grouped subsections. Context collection comes first; the later phases --- hypothesis refinement, experiment design, synthetic preflight, and report generation --- show how each step builds from initial information-gathering to create a customized blueprint for implementing and deploying the experiment in UpGrade. We describe each phase in detail below.

\subsection{Context Collection}

The conversation opens by collecting basic context about the EdTech application and how it works, such as what the application does, who its learners are, the domain, and a description of typical user flow, followed by "ideation coaching", which gathers information about whether the user is interested in solving a specific problem, developing an intervention, or creating a feature, and then helping to identify where in the app's interaction flow an experiment might run. Users can upload files such as screenshots or documents to help the AI provide more specific follow-up questions and suggestions.

\subsection{Ideation and Hypothesis Refinement}

This is the most dynamic phase. The user may arrive with a clear idea (``test whether adding a hint improves correctness rates''), an unfocused problem to solve (``students get stuck here''), or need more directed guidance on idea generation. The AI helps to refine ideas when needed, then proposes candidate interventions, providing comparison between alternative approaches if desired. During this phase, the system suggests outcome metrics based on contextual information gathered in the previous phase, then refines the approved approaches into a testable hypothesis, a sort of lightweight version of the \emph{generate $\rightarrow$ critique $\rightarrow$ refine $\rightarrow$ refine} pattern in the AI co-scientist model ~\cite{ai-coscientist}. Part of this flow may involve grounding ideas in prior research; users can request that the AI provide citations and background information that is then added to the final report.
...
With an approved design, the AI offers an optional synthetic preflight simulation against the hosted UpGrade demo backend, summarizing enrollment per condition, condition-level metric values, and basic interpretation cues. We make the caveat explicit: this is a \emph{preflight demonstration of UpGrade mechanics}, not evidence of the intervention's effect on learning outcomes. Rather, its purpose is to help users build familiarity with the UpGrade UI and understand what their experiment might look like in the platform environment. We expect that this feature will primarily be of interest to users who have never used UpGrade before, or who have very limited experience with the platform. As users become more acquainted with UpGrade's interface by running real experiments, this step may be skipped.

\subsection{Report Generation}

In the final phase, the AI assembles a detailed markdown report covering the learning app description and other relevant context, a refined hypothesis, proposed UpGrade experiment design, simulation summary (if this feature was used), an implementation to-do list, and templated setup and client-integration guidance with code examples. The report represents the primary handoff artifact across user roles and AI tools (if used). Since the initial prototype does not have a persistent database, reports are not currently saved across sessions. However, a user who is interested in iterative experimentation can upload the report as part of context collection in a new session, providing background information for the AI without the need to engage in redundant dialogue. 
...
\textbf{Walkthrough.} After describing the app (including a screenshot of the problem page), the primary concern, and the student population targeted, the user (for instance, a product manager), and the AI develop and refine a hypothesis: \emph{``Adding an optional hint button on this problem page will increase the correctness rate at first attempt without substantially increasing time-on-task.''} The AI proposes correctness rate and time-on-task as metrics; on approval from the product manager, it produces the experiment design in Figure~\ref{fig:design}.
...
Metrics:
  completionRate: Percent = CORRECT
  timeOnTask:     Mean
\end{verbatim}
\caption{Proposed UpGrade experiment design generated by the AI consultant for the ExampleMathApp hint-button scenario, handed off in the final report alongside hypothesis, simulation summary, implementation to-do list, and client-integration code guidance.}
\Description{A code-style listing showing the proposed UpGrade experiment configuration: name, description, app context, decision point (site and target), two conditions split 50/50, participants set to Include All, and two metrics (completion rate and time on task).}
\label{fig:design}
\end{figure}

A synthetic simulation with 200 participants returns balanced enrollment (control 97, hint\_button 103), correctnessRate 78.4 for control vs.\ 84.5 for hint-hint\_button, and timeOnTask 11.2 vs.\ 12.6. The simulation suggests that \emph{hint\_button} trends higher on correctness in this synthetic run, but notes that this is not meant to replace a true randomized experiment. The final report bundles the app and page descriptions, hypothesis, UpGrade experiment design, simulation summary with caveat, implementation to-do list (add the hint component in the client code, insert UpGrade API calls for condition assignment and metric collection, gate hint rendering on the assigned condition), and client-integration code examples.
